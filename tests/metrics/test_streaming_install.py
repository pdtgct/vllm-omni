...
def test_install_with_log_stats_false_keeps_scrape_sample_free_and_accepts_events() -> None:
    ...
    handle = observer.unit_ready(
        session_key="install-scope-key", cadence_ms="560", chunk_type="regular", ready_stamp_s=0.0
    )
...
def test_route_setup_injects_the_installed_observer_into_native_session_construction() -> None:
    ...
    session = NemotronRealtimeSession.from_model_config(hf_config, observer=observer, session_key="route-inject-key")
