        ...
        if isinstance(node, ast.Import):
            for imported in node.names:
                ...
                if root_module in PY311_MODULES:
                    findings.append(f"{filename}:{node.lineno}: import {root_module}")
            ...
            for imported in node.names:
                if imported.name in forbidden:
                    findings.append(f"{filename}:{node.lineno}: from {root_module} import {imported.name}")

    for node in ast.walk(tree):
        if not isinstance(node, ast.Attribute) or not isinstance(node.value, ast.Name):
            continue
        module = module_aliases.get(node.value.id)
        if module is not None and node.attr in PY311_SYMBOLS.get(module, frozenset()):
            findings.append(f"{filename}:{node.lineno}: {node.value.id}.{node.attr}")
...
def test_python310_scan_rejects_runtime_incompatibilities(source: str, expected: str) -> None:
    # @spec ENV-MIG-004
    assert any(expected in finding for finding in _python311_references(source, "probe.py"))
...
def test_python310_compatibility_modules_import_with_stable_semantics() -> None:
    ...
    comfy_types = _load_source_module(
        ...
        ROOT / "apps" / "ComfyUI-vLLM-Omni" / "comfyui_vllm_omni" / "utils" / "types.py",
    )
