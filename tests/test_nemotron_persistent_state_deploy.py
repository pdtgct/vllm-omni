...
def _pipeline() -> PipelineConfig:
    pipeline = config_factory.resolve_pipeline_config("nemotron_asr")
    if pipeline is None:
...
def _persistent_state(stage: Any) -> dict[str, Any]:
    ...
    if isinstance(value, type) or not is_dataclass(value) or type(value).__name__ != "PersistentStateDeployConfig":
...
class TestNemotronPersistentStateDeploy:
    ...
    def test_lowered_envelope_participates_in_vllm_execution_identity(
        ...
        api_config = VllmConfig(additional_config=dict(lowered), device_config=DeviceConfig("cpu"))
        core_config = VllmConfig(additional_config=dict(lowered), device_config=DeviceConfig("cpu"))
        changed = VllmConfig(
            additional_config={
                **dict(lowered),
                "persistent_state_admission_retry_floor_ms": 125,
    ...
    def test_missing_packaged_nemotron_deploy_has_no_generic_fallback(
        ...
        try:
            StageConfigFactory._create_legacy_from_registry(_pipeline(), {})
