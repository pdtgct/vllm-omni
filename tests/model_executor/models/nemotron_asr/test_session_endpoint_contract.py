...
def test_factory_rejects_missing_colliding_or_label_space_control(overrides: dict[str, Any]) -> None:
...
def test_segmenter_delegates_to_one_session_owned_audio_authority() -> None:
    ...
    async def scenario() -> None:
        chunk_samples = session.AdmittedGeometry.from_cadence("560ms").chunk_samples
...
def test_session_control_methods_delegate_to_one_authority_without_second_lock() -> None:
    ...
    samples: np.ndarray[Any, np.dtype[np.float32]] = np.arange(
