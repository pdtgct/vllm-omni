...
def _nemo_reference(
    ...
    prompt = torch.zeros(batch, time, _PROMPTS, dtype=encoded.dtype, device=encoded.device)
    ...
    out = conditioner.prompt_kernel(torch.cat([encoded, prompt], dim=-1)).to(out_dtype)
...
def test_matches_nemo_reference_layout_for_layout(conditioner):
    ...
    reference = _nemo_reference(conditioner, encoded_btd.transpose(1, 2), index=3).transpose(1, 2)
