...
def _fresh_pools(
    *, n_layers: int, window: int, d_model: int, pred_hidden: int, cap: int, num_blocks: int, device: str
    ...
    return {
        ...
        "time_pools": [
            torch.zeros(num_blocks, d_model, CONV_KERNEL - 1, device=device) for _ in range(n_layers)
        ],
        "len_pools": [
...
def _build_execution_fingerprint(
    *, device: str, model_artifact_digest: str, repo_root: Path,
    ...
    return {
        ...
        "compute_capability": (
            ".".join(str(v) for v in torch.cuda.get_device_capability(0)) if is_cuda else ""
        ),
        "python_build": ".".join(str(v) for v in sys.version_info[:3]),
        "torch_build": torch.__version__,
        "accel_library_versions": (
            [["cuda", str(torch.version.cuda)]] if is_cuda and torch.version.cuda else []
        ),
...
class _SessionDriver:
    ...
    def __init__(
        self, *, core: NemotronASRCore, cfg: dict[str, Any], geometry_id: int,
        request_id: str, device: str,
    ) -> None:
        self.core = core
        self.device = device
        ...
        self.pools = _fresh_pools(
            n_layers=int(cfg["n_layers"]), window=int(cfg["att_context_left"]),
            d_model=int(cfg["d_model"]), pred_hidden=int(cfg["pred_hidden"]),
            cap=int(manifests.SESSION_LIMITS["queue_capacity"]), num_blocks=num_blocks,
            device=device,
        )
        ...
        self.sink = commit_sink_mod.BoundedCommitSink(
            self.registry, max_rows=1, max_capture_rows=1,
            max_capture_bytes=1 << 30, device=torch.device(device),
        )
        self.staging = advance.HostStaging(1)
        self.adapter = advance.make_mrv1_adapter(
            hidden_size=self.hidden, park_id=self.park_id, blank_id=core.blank_id
        )
    ...
    def _replay_turn(self, row: plan_mod.ObservedRow) -> tuple[int, list[advance.CaptureRecord]]:
        ...
        context = plan_mod.prepare_plan_context(
            self.registry, [row], resident_request_ids=[row.request_id],
            placeholder_id=self.placeholder_id, num_prompts=self._num_prompts,
            num_geometries=len(manifests.CADENCES), now_ns=self._now_ns, step=self._step,
        )
        rp = plan_mod.build_row_plan(
            context, num_decodes=1, num_prefills=0,
            null_block_id=NULL_BLOCK_ID, num_pool_blocks=self.num_blocks,
        )
        ...
        projected = advance.advance_model_rows(
            self.core, input_ids, embeds, rp,
            adapter=self.adapter, decode_resolver=self._resolver,
            placeholder_id=self.placeholder_id, park_id=self.park_id,
    ...
    def run_chunk(
        self, *, samples: torch.Tensor, seq: int, is_final_tail: bool, prompt: int,
        ...
        self._step += 1
        context = plan_mod.prepare_plan_context(
            self.registry, [row], resident_request_ids=[self.request_id],
            placeholder_id=self.placeholder_id, num_prompts=self._num_prompts,
            num_geometries=len(manifests.CADENCES), now_ns=self._now_ns, step=self._step,
        )
        rp = plan_mod.build_row_plan(
            context, num_decodes=0, num_prefills=1,
            null_block_id=NULL_BLOCK_ID, num_pool_blocks=self.num_blocks,
        )
        input_ids = torch.tensor([self.placeholder_id], dtype=torch.long, device=self.device)
        projected = advance.advance_model_rows(
            self.core, input_ids, embeds_row.unsqueeze(0), rp,
            adapter=self.adapter, decode_resolver=self._resolver,
            placeholder_id=self.placeholder_id, park_id=self.park_id,
            commit_sink=self.sink, capture=True, graph_covers_decode=False,
            staging=self.staging, **self.pools,
...
def _run_cadence(
    *, core: NemotronASRCore, cfg: dict[str, Any], cadence: str, samples: torch.Tensor,
    ...
    driver = _SessionDriver(
        core=core, cfg=cfg, geometry_id=geometry_id,
        request_id=f"p7-{clip_id}-{cadence}", device=device,
    )
    ...
    def _commit_chunk(*, seq: int, chunk_samples: torch.Tensor, is_final_tail: bool) -> None:
        ...
        record, labels = driver.run_chunk(
            samples=chunk_samples, seq=seq, is_final_tail=is_final_tail, prompt=prompt_index,
        )
        ...
        geom = chunk_delta_geometry(
            chunk_sequence=seq, mel_length=mel_length, encoder_length=encoder_length,
            is_final_tail=is_final_tail,
        )
        chunk_index = len(chunk_records)
        chunk_records.append(
            build_chunk_record(
                chunk_index=chunk_index, geometry=geom,
                encoder_length=encoder_length, is_final_tail=is_final_tail,
            )
        )
        if geom.has_tensors:
    ...
    for i in range(num_regular):
        ...
        _commit_chunk(
            seq=i, chunk_samples=samples[start : start + cadence_samples], is_final_tail=False,
        )
    ...
    manifest = build_manifest(
        ...
        chunk_timing_ms=chunk_timing_ms, chunk_records=chunk_records,
    )
    (out_dir / "manifest.json").write_text(json.dumps(manifest, indent=2))
    return {
        "cadence": cadence, "num_chunks": len(chunk_records),
        "final_transcript": manifest["final_transcript"],
        "tensors_size_bytes": tensors_size_bytes, "out_dir": str(out_dir),
    }
    ...
    for cadence in cadences:
        try:
            report["cadences"][cadence] = _run_cadence(
                core=core, cfg=cfg, cadence=cadence, samples=waveform,
                prompt_index=prompt_index, device=args.device, checkpoint_dir=args.checkpoint,
                clip_path=args.clip, clip_id=clip_id, out_root=args.out,
