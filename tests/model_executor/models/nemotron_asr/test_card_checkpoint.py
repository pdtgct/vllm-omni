...
def test_card_serving_config_constructs_and_validates() -> None:
    ...
    again = NemotronCardServingConfig(**{k: v for k, v in config.to_dict().items() if k != "encoder_config"})
...
def test_prompt_dictionary_loads_from_the_processor_sidecar(tmp_path) -> None:
    (tmp_path / "processor_config.json").write_text(json.dumps({"prompt_dictionary": {"en-US": 0, "auto": 101}}))
    config = NemotronCardServingConfig(**_card_config_dict())
    ...
    (tmp_path / "processor_config.json").write_text(json.dumps({"prompt_dictionary": {"clobber": 1}}))
    ensure_prompt_dictionary(config, tmp_path)
    assert config.prompt_dictionary == {"en-US": 0, "auto": 101}
...
def test_prompt_dictionary_hydrates_on_the_autoconfig_dispatch_path(tmp_path) -> None:
    ...
    """
    (tmp_path / "processor_config.json").write_text(json.dumps({"prompt_dictionary": {"en-US": 0, "auto": 101}}))
    config = NemotronCardServingConfig.from_dict(_card_config_dict(), name_or_path=str(tmp_path))
    assert config.prompt_dictionary == {"en-US": 0, "auto": 101}
...
def test_synthesized_filterbank_matches_librosa_when_available() -> None:
    ...
    reference = torch.from_numpy(
        librosa.filters.mel(sr=16000, n_fft=512, n_mels=128, fmin=0.0, fmax=8000.0, norm="slaney")
    ).to(torch.float32)
...
def test_cadence_admission_follows_the_declared_set_not_the_code() -> None:
    ...
    for cadence in ("80ms", "320ms", "560ms", "1120ms"):
        session = NemotronRealtimeSession.from_model_config(config, cadence=cadence)
        assert session.geometry.cadence == cadence

    undeclared = _admissible_config()
    del undeclared.supported_num_lookahead_tokens
    session = NemotronRealtimeSession.from_model_config(undeclared, cadence="160ms")
