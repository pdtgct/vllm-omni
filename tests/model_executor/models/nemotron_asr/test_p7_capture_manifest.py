...
def test_session_first_chunk_has_no_prefix_to_strip() -> None:
    ...
    geom = chunk_delta_geometry(chunk_sequence=0, mel_length=14, encoder_length=6, is_final_tail=False)
...
def test_continuing_chunk_strips_nine_frame_prefix() -> None:
    ...
    geom = chunk_delta_geometry(chunk_sequence=3, mel_length=16, encoder_length=8, is_final_tail=False)
...
def test_zero_commit_final_tail_has_no_tensors() -> None:
    ...
    geom = chunk_delta_geometry(chunk_sequence=5, mel_length=0, encoder_length=0, is_final_tail=True)
...
def test_zero_commit_non_final_chunk_still_reports_has_tensors() -> None:
    ...
    geom = chunk_delta_geometry(chunk_sequence=2, mel_length=0, encoder_length=0, is_final_tail=False)
    assert geom.has_tensors is True
    assert geom.new_mel_frames == 0
...
def test_short_final_tail_with_real_content_keeps_tensors() -> None:
    ...
    geom = chunk_delta_geometry(chunk_sequence=4, mel_length=11, encoder_length=1, is_final_tail=True)
    assert geom.mel_start == 9
...
def test_mel_length_below_prefix_clamps_rather_than_going_negative() -> None:
    ...
    geom = chunk_delta_geometry(chunk_sequence=1, mel_length=4, encoder_length=0, is_final_tail=False)
    assert geom.new_mel_frames == 0
...
def test_negative_inputs_raise(chunk_sequence: int, mel_length: int, encoder_length: int) -> None:
...
def test_build_chunk_record_has_exactly_the_required_keys() -> None:
    geom = chunk_delta_geometry(chunk_sequence=1, mel_length=16, encoder_length=8, is_final_tail=False)
    record = build_chunk_record(chunk_index=1, geometry=geom, encoder_length=8, is_final_tail=False)
    assert set(record) == {
        "chunk_index",
...
def test_build_chunk_record_zeroes_encoder_frames_when_no_tensors() -> None:
    geom = chunk_delta_geometry(chunk_sequence=6, mel_length=0, encoder_length=0, is_final_tail=True)
    record = build_chunk_record(chunk_index=6, geometry=geom, encoder_length=0, is_final_tail=True)
...
def test_build_chunk_record_rejects_negative_index() -> None:
    geom = chunk_delta_geometry(chunk_sequence=0, mel_length=1, encoder_length=1, is_final_tail=False)
    with pytest.raises(ValueError):
        build_chunk_record(chunk_index=-1, geometry=geom, encoder_length=1, is_final_tail=False)
...
def _write_checkpoint_identity_fixture(tmp_path: Path) -> None:
    ...
    (tmp_path / "config.json").write_text(
        json.dumps({"prompt_dictionary": prompt_dictionary, "num_prompts": 128}),
        encoding="utf-8",
    )
    ...
    (tmp_path / "checkpoint-profile.json").write_text(json.dumps(profile), encoding="utf-8")
...
def test_checkpoint_identity_binds_source_derived_manifests_and_prompt(
    ...
    assert identity.derived_model_digest == sha256_file(tmp_path / "model.safetensors")
...
@pytest.mark.parametrize(
    ...
    (
        ...
        lambda root: (root / "state-manifest.json").write_text(json.dumps({"schema": "changed"}), encoding="utf-8"),
        lambda root: (root / "config.json").write_text(
            json.dumps({"prompt_dictionary": {"en-US": 3}, "num_prompts": 128}),
            encoding="utf-8",
        ),
    ),
)
def test_checkpoint_identity_rejects_content_drift(tmp_path: Path, mutate: object) -> None:
    _write_checkpoint_identity_fixture(tmp_path)
...
def test_checkpoint_identity_rejects_invalid_prompt_row_bounds(tmp_path: Path, config: dict[str, object]) -> None:
    _write_checkpoint_identity_fixture(tmp_path)
...
def test_capture_qualification_inputs_fail_closed(overrides: dict[str, object], message: str) -> None:
...
def test_build_manifest_contains_all_required_keys() -> None:
    geom = chunk_delta_geometry(chunk_sequence=0, mel_length=5, encoder_length=2, is_final_tail=True)
    record = build_chunk_record(chunk_index=0, geometry=geom, encoder_length=2, is_final_tail=True)
