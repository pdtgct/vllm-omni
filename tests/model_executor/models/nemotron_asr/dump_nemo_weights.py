...
def main() -> None:
    ...
    model = ASRModel.restore_from(restore_path=str(args.model), map_location=torch.device("cpu"))
    model.eval()
    ...
    tok = model.tokenizer
    spm_path = getattr(tok, "model_path", None) or getattr(tok.tokenizer, "model_path", None)
    if spm_path:
    ...
    meta = {
        ...
        "prompt_dictionary": dict(model.cfg.model_defaults.get("prompt_dictionary", {})),
