...
def main() -> None:
    ...
    with torch.inference_mode():
        mel, mel_len = featurizer(waveform, torch.tensor([waveform.shape[1]], device=device))
    ...
    with torch.inference_mode():
        for chunk in range(n_chunks):
            ...
            prefix = mel[:, :, :mel_end]
            enc_out, enc_len = encoder(prefix, torch.tensor([mel_end], device=device))
            ...
            labels, decode_state = greedy_decode_chunk(cond[0], predictor, joint, decode_state)
    ...
    print(f"partial cadence: {n_chunks - mismatches}/{n_chunks} identical")
