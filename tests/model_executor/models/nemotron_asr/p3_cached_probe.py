...
def main() -> None:
    ...
    core, meta = load_core_from_dump(args.dump, device=device, att_context=att_context)
    ...
    waveform = torch.from_numpy(pcm.astype(np.float32) / 32768.0).unsqueeze(0).to(device)
    with torch.inference_mode():
        mel, mel_len = core.featurizer(waveform, torch.tensor([waveform.shape[1]], device=device))
        ...
        for chunk in range(n_chunks):
            ...
            if chunk == 0:
                ...
                if start < 0:
                    ...
                    pad = torch.zeros(1, mel.shape[1], -start, device=device)
                    chunk_mel = torch.cat([pad, mel[:, :, :mel_end]], dim=2)
                else:
                    chunk_mel = mel[:, :, start:mel_end]
                drop = DROP_EXTRA
            frames = stream_step(core.encoder, chunk_mel, caches, drop_extra=drop)
            if frames.shape[1] > 0:
    ...
    mismatches = sum(ours != golden for ours, golden in zip(partials, golden_partials))
    ...
    print(f"cached-path: final {'MATCH' if final_match else 'MISMATCH'}; cadence {n_chunks - mismatches}/{n_chunks}")
