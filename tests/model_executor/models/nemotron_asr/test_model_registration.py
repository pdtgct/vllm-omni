...
@pytest.mark.parametrize("conflicting_field", ["module_name", "class_name"])
def test_registration_replaces_conflicting_targets(monkeypatch, conflicting_field) -> None:
    ...
    target = {"module_name": _LAZY_MODULE, "class_name": "NemotronASRForRNNT"}
    target[conflicting_field] = "upstream_target"
    monkeypatch.setitem(ModelRegistry.models, _ARCHITECTURE, SimpleNamespace(**target))
    register_omni_models_to_vllm()
    registered = ModelRegistry.models[_ARCHITECTURE]
    assert registered.module_name == _LAZY_MODULE
    assert registered.class_name == "NemotronASRForRNNT"
    register_omni_models_to_vllm()
    assert ModelRegistry.models[_ARCHITECTURE] is registered
