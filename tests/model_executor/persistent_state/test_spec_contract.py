...
def test_persistent_spec_is_direct_immutable_core_kv_spec() -> None:
    ...
    for descriptor in descriptors:
        ...
        initializer = descriptor_field(descriptor, "initializer", "fresh_initializer", "fresh_init")
    ...
    assert (
        spec.page_size_bytes % max(descriptor_field(item, "alignment_bytes", "alignment") for item in descriptors) == 0
    )
...
def test_spec_schema_fingerprint_covers_complete_generic_layout() -> None:
    ...
    assert all(
        not hasattr(base, request_field) for request_field in ("request_id", "session_id", "generation", "slot_id")
    )
