...
def require_persistent_state_module() -> ModuleType:
    ...
    try:
        ...
        _fail(f"Missing or unusable future persistent-state seam {PERSISTENT_STATE_MODULE!r}: {exc}")
...
def _select_parameter(parameters: Mapping[str, inspect.Parameter], aliases: Sequence[str]) -> str | None:
...
def make_generic_spec(module: ModuleType, variant: str = "base") -> Any:
    ...
    return _construct(
        spec_cls,
        {
            "descriptors": (("descriptors", "tensor_descriptors", "layout"), (descriptor_one, descriptor_two)),
...
def state_binding_pair(manager: Any, request_id: str) -> tuple[Any, Any]:
    ...
    for name in ("get_state_binding", "get_binding"):
        ...
        if callable(candidate):
            ...
            break
    if method is None:
        _fail("PersistentStateManager must expose a reviewed binding accessor (expected get_state_binding(request_id))")
...
def _call_payload_validator(fn: Callable[..., Any], payload: Any, label: str) -> Any:
...
def invoke_connector_operation(connector: Any, operation: str) -> Any:
    ...
    if not callable(method):
        _fail(
            f"PersistentStateConnector must expose a reviewed {operation!r} operation (or request/execute dispatcher)"
        )
