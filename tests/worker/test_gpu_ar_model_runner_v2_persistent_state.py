...
def test_core_call_order_guard_matches_the_selected_v029_pin() -> None:
...
def _native_empty_block_tables() -> BlockTables:
    # Exercise native zero-group methods with CPU storage, without pretending
    # to validate the GPU UVA allocator used by the full constructor.
    tables = object.__new__(BlockTables)
    tables.num_kv_cache_groups = 0
    tables.input_block_tables = []
    tables.slot_mappings = torch.zeros((0, 16), dtype=torch.int64)
    return tables
...
def test_persistent_only_block_tables_cross_the_real_core_staged_write_step() -> None:
    ...
    runner.block_tables = _native_empty_block_tables()
    ...
    runner.kv_connector = SimpleNamespace(no_forward=lambda output: EMPTY_MODEL_RUNNER_OUTPUT)
    runner.ec_connector = SimpleNamespace(no_forward=lambda output: SimpleNamespace(ec_connector_output=None))
    actual = type(runner).execute_model(runner, _scheduler_output(total_tokens=0))
    assert actual is EMPTY_MODEL_RUNNER_OUTPUT
    assert type(runner.block_tables) is BlockTables


def test_persistent_only_block_tables_expose_empty_attention_views() -> None:
    # @spec PORT-MIG-005 / PORT-STATE-002
    tables = _native_empty_block_tables()
    tables.apply_staged_writes()
    tables.append_block_ids(0, (), overwrite=True)
    assert tables.gather_block_tables(object(), 4) == ()
    assert tables.get_dummy_block_tables(4) == ()
    assert tables.compute_slot_mappings(object(), object(), object(), 7).shape == (0, 7)
    assert tables.get_dummy_slot_mappings(5).shape == (0, 5)
...
def test_worker_selects_v2_before_model_state_layers_exist(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(RuntimeError, match="requires Model Runner v2"):
        worker._select_model_runner_cls()
...
def test_persistent_only_worker_skips_core_text_cache_warmup(
    ...
    def persistent_warmup(self):
        events.append("persistent-only")
        return marker

    monkeypatch.setattr(worker_cls, "_compile_or_warm_up_persistent_only_model", persistent_warmup)
...
def test_nonpersistent_worker_preserves_core_warmup(
    ...
    def core_warmup(self):
        events.append("core")
        return marker

    monkeypatch.setattr(CoreGPUWorker, "compile_or_warm_up_model", core_warmup)
...
def test_persistent_only_warmup_preserves_the_core_operational_postamble(
    ...
    assert callable(warmup_attestation), "PORT-ADV-003 missing worker warmup attestation"
...
def test_persistent_only_rejects_extra_ordinary_block_ids() -> None:
    # @spec PORT-MIG-005 / PORT-STATE-002
    runner = _runner()
    runner._persistent_state_group_index = 0
    runner._ordinary_kv_cache_group_count = 0
    assert runner._ordinary_block_ids(([1],)) == ()
    with pytest.raises(RuntimeError, match="ordinary block ids"):
        runner._ordinary_block_ids(([1], [2]))


def test_postamble_failure_does_not_attest_resident_warmup(monkeypatch: pytest.MonkeyPatch) -> None:
    # @spec ENV-MIG-012 / PORT-INT-013
    from vllm.config.compilation import CompilationMode

    module = importlib.import_module("vllm_omni.worker.gpu_ar_worker")
    worker = _worker(persistent=True, ordinary_groups=0)
    worker.model_config = SimpleNamespace(enforce_eager=True, seed=17)
    worker.compilation_config = SimpleNamespace(mode=CompilationMode.NONE)
    worker.observability_config = SimpleNamespace(jit_monitor_mode="off", jit_monitor_verbose=False)
    worker.model_runner.lora_config = None
    worker.model_runner.maybe_remove_all_loras = lambda config: None
    worker.model_runner.model.warmup_resident_state = lambda: None
    for name in ("kernel_warmup", "set_random_seed", "freeze_gc_heap", "maybe_attach_gc_debug_callback"):
        monkeypatch.setattr(module, name, lambda *args, **kwargs: None)
    monkeypatch.setattr(importlib.import_module("vllm.utils.jit_monitor"), "activate", lambda **kwargs: None)

    def fail_postamble() -> None:
        raise RuntimeError("postamble failed")

    monkeypatch.setattr(module, "enable_gpu_sync_check", fail_postamble)
    with pytest.raises(RuntimeError, match="postamble failed"):
        type(worker)._compile_or_warm_up_persistent_only_model(worker)
    assert worker.persistent_state_warmup_attestation() is False
