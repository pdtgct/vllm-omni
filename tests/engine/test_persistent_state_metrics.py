...
def test_snapshot_reserve_and_release_project_manager_inventory() -> None:
    ...
    async def scenario() -> None:
        ...
        metrics = OmniStreamingMetrics(model_name="state-service-projection", log_stats=True)
...
def test_definitive_capacity_failure_is_counted_once() -> None:
    ...
    async def scenario() -> None:
        stage = _StageClient()
        service = PersistentStateService(stage)
        metrics = OmniStreamingMetrics(model_name="state-service-rejection", log_stats=True)
        await service.check_health()
        service.install_metrics(metrics)
        stage.capacity_error = True
        prefix = (
            f'{defs.STREAMING_ADMISSION_REJECTIONS}_total{{model_name="state-service-rejection",reason="capacity"}}'
        )
...
def test_unsupported_interval_failure_is_counted_once() -> None:
    ...
    async def scenario() -> None:
        ...
        stage = _StageClient()
        service = PersistentStateService(stage)
        metrics = OmniStreamingMetrics(model_name="state-service-unsupported", log_stats=True)
        await service.check_health()
        service.install_metrics(metrics)
        stage.reserve_error = RuntimeError(
            "persistent_state_unsupported_service_interval: 80ms exceeds the empty-pool transaction envelope"
        )
        prefix = (
...
def test_retryable_shed_uses_capacity_metric_reason() -> None:
    ...
    async def scenario() -> None:
        stage = _StageClient()
        service = PersistentStateService(stage)
        metrics = OmniStreamingMetrics(model_name="state-service-shed", log_stats=True)
        await service.check_health()
        service.install_metrics(metrics)
        stage.reserve_error = RuntimeError("persistent_state_horizon_exhausted: retry after tombstone expiry")
        prefix = f'{defs.STREAMING_ADMISSION_REJECTIONS}_total{{model_name="state-service-shed",reason="capacity"}}'
        before = _value(prefix) or 0.0
...
def test_readiness_probe_is_not_an_admission_rejection() -> None:
    ...
    async def scenario() -> None:
        stage = _StageClient()
        service = PersistentStateService(stage)
        metrics = OmniStreamingMetrics(model_name="state-service-health", log_stats=True)
        await service.check_health()
        service.install_metrics(metrics)
        service.close_admission()
        prefix = (
            f'{defs.STREAMING_ADMISSION_REJECTIONS}_total{{model_name="state-service-health",reason="unavailable"}}'
        )
        before = _value(prefix) or 0.0
