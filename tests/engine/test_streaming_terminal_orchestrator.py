...
class _TerminalPool:
    ...
    def _infer_audio_sample_rate(self) -> int:
        return 16000
...
def _request_state(request_id: str) -> OrchestratorRequestState:
    ...
    state.running_counter_registered = True
    return state
...
async def test_parked_completion_routes_one_stop_and_cleans_only_owner() -> None:
    ...
    orchestrator.request_states["done"].streaming.segments[0] = StreamingSegmentState(finished=True)
