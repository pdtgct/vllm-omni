...
def test_waiter_entry_is_fixed_shape_and_carries_no_audio_or_session() -> None:
    ...
    assert not field_names.intersection({"audio", "audio_bytes", "decoded_audio", "session", "error_text"})
...
def test_all_arrivals_enter_the_queue_and_duplicate_connection_coalesces() -> None:
    ...
    assert [(item.attempt_id, item.outcome) for item in dispositions] == [("attempt-1", "shed")]
...
def test_reused_slab_slot_fences_stale_handle_and_deadline() -> None:
    ...
    stale = controller.enqueue(_attempt(1, admission_deadline_ns=5, unadmitted_deadline_ns=50))
    controller.cancel(stale)
    current = controller.enqueue(_attempt(2, admission_deadline_ns=20, unadmitted_deadline_ns=50))
    ...
    clock.now_ns = 21
    dispositions = controller.expire_due()
    assert [(item.attempt_id, item.outcome) for item in dispositions] == [("attempt-2", "shed")]
...
def test_cancel_or_expiry_releases_the_protected_head(
    ...
    clock = _Clock()
    controller = _controller(clock, max_inflight_reserves=1, dispatch_budget=1)
    protected = controller.enqueue(_attempt(1, interval_ms=80, admission_deadline_ns=20))
    ...
    if terminal == "cancel":
        ...
        clock.now_ns = 21
        dispositions = controller.expire_due()
        assert [(d.attempt_id, d.outcome) for d in dispositions] == [("attempt-1", "shed")]
...
def test_waiter_expiring_without_authority_is_unavailable_not_shed() -> None:
    ...
    clock = _Clock()
    controller = _controller(clock)
    controller.enqueue(_attempt(1, admission_deadline_ns=10, unadmitted_deadline_ns=100))
    controller.authority_lost(engine_epoch="epoch-a")
    clock.now_ns = 11

    dispositions = controller.expire_due()

    assert [(item.attempt_id, item.outcome) for item in dispositions] == [("attempt-1", "unavailable")]
...
def test_total_connection_deadline_is_terminal_even_before_wait_deadline() -> None:
    ...
    clock = _Clock()
    controller = _controller(clock)
    controller.enqueue(_attempt(1, admission_deadline_ns=100, unadmitted_deadline_ns=10))
    clock.now_ns = 11
