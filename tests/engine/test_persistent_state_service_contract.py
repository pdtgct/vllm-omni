...
def test_refusal_types_encode_retryability_without_message_matching() -> None:
    ...
    assert unsupported_cls is not None, "PORT-STATE-024 missing unsupported-service-interval type"
...
def _publish_warmup_attestation(core: Any) -> None:
    ...
    assert callable(publish), "ENV-MIG-012 missing worker-to-manager warmup attestation join"
    publish()
...
def test_engine_core_init_automatically_publishes_worker_warmup_attestation(
    ...
    def fake_base_init(core: Any, *args: Any, **kwargs: Any) -> None:
        ...
        def collective_rpc(method: str) -> list[bool]:
...
class _TombstoneStage:
    ...
    async def call_utility_async(self, name: str, *args: Any) -> dict[str, Any] | bool:
...
def test_runtime_config_requires_the_complete_a36_envelope() -> None:
    ...
    defaults_sources: tuple[None, dict[str, Any]] = (None, {})
    for defaults_source in defaults_sources:
        try:
