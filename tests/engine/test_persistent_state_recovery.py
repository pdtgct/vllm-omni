...
def test_bootstrap_profile_seal_is_one_shot_and_opens_public_authority() -> None:
    ...
    async def scenario() -> None:
        ...
        assert callable(bootstrap) and callable(seal), "ENV-MIG-012 missing bootstrap/profile-seal lifecycle"
        ...
        assert callable(reserve_for_priming), "PORT-PERF-005 missing private bootstrap reserve"
        with pytest.raises(
            PersistentStateServiceUnavailable,
...
def test_hard_cap_service_seals_static_authority_without_profile() -> None:
    ...
    async def scenario() -> None:
        ...
        assert callable(derive), "PORT-STATE-027 missing static hard-cap authority derivation"
        admission_config = AdmissionControllerConfig(
            waiter_capacity=4,
            max_inflight_reserves=2,
            dispatch_budget=1,
