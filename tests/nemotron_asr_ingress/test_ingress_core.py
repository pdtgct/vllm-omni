...
# @spec ING-CORE-005
async def test_partials_carry_the_cumulative_hypothesis() -> None:
    core, fake, _ = make_core()
    core.configure(CFG, now=0.0)
    events = await core.receive_audio(audio(2 * CHUNK_SAMPLES), now=0.1)
...
# @spec ING-LIFE-002
async def test_exactly_one_final_per_session() -> None:
    core, fake, _ = make_core()
    core.configure(CFG, now=0.0)
...
async def test_audio_after_finalize_answers_session_terminal() -> None:
    core, fake, _ = make_core()
    core.configure(CFG, now=0.0)
    await core.finalize(now=0.1)
    answer = await core.receive_audio(audio(CHUNK_SAMPLES), now=0.2)
    assert len(answer) == 1
...
# @spec ING-ERR-004
async def test_second_finalize_answers_session_terminal_never_silence() -> None:
    core, _, _ = make_core()
    core.configure(CFG, now=0.0)
    await core.finalize(now=0.1)
    answer = await core.finalize(now=0.2)
...
# @spec ING-LIFE-003
async def test_zero_audio_finalize_is_a_valid_degenerate_session() -> None:
    core, fake, _ = make_core()
    core.configure(CFG, now=0.0)
...
# @spec ING-LIFE-004
async def test_close_frees_the_slot_and_aborts_immediately() -> None:
    core, fake, gate = make_core()
    core.configure(CFG, now=0.0)
    assert gate.active == 1
    await core.close(now=0.1)
    assert gate.active == 0
    assert fake.aborted


# @spec ING-LIFE-005, ING-LIFE-006
async def test_idle_ttl_backstop_aborts_with_idle_timeout() -> None:
    values = make_values(idle_ttl_s=60.0)
    core, fake, gate = make_core(values=values)
    core.configure(CFG, now=0.0)
    (abort,) = await core.poll(now=60.1)
...
# @spec ING-LIFE-005
async def test_receive_cadence_resets_the_idle_clock() -> None:
    values = make_values(idle_ttl_s=60.0)
    core, _, _ = make_core(values=values)
    core.configure(CFG, now=0.0)
    await core.receive_audio(audio(100), now=50.0)
    assert await core.poll(now=100.0) == []
    assert await core.poll(now=110.5) != []


# @spec ING-LIFE-007
async def test_valid_locale_update_is_acked_and_forwarded() -> None:
    core, fake, _ = make_core()
    core.configure(CFG, now=0.0)
    events = await core.update({"target_lang": "es-US"}, now=0.1)
    assert events == [UpdateAck(honored={"target_lang": "es-US"})]
    assert fake.locales == ["es-US"]


# @spec ING-LIFE-007
async def test_unknown_locale_update_names_the_field_and_continues() -> None:
    core, fake, _ = make_core()
    core.configure(CFG, now=0.0)
    (err,) = await core.update({"target_lang": "xx-XX"}, now=0.1)
    ...
    events = await core.receive_audio(audio(CHUNK_SAMPLES), now=0.2)
    assert any(isinstance(e, Partial) for e in events)


# @spec ING-LIFE-008
async def test_admission_fixed_field_update_is_rejected_with_continuation() -> None:
    core, fake, _ = make_core()
    core.configure(CFG, now=0.0)
    (err,) = await core.update({"chunk_ms": 320}, now=0.1)
    assert isinstance(err, SessionError)
...
# @spec ING-LIFE-009
async def test_mixed_update_rejects_first_then_acks_the_honored_subset() -> None:
    core, _, _ = make_core()
    core.configure(CFG, now=0.0)
    events = await core.update(
        {"target_lang": "es-US", "chunk_ms": 320}, now=0.1
    )
    assert len(events) == 2
...
# @spec ING-FE-005
async def test_oversized_burst_answers_buffer_overflow() -> None:
    values = make_values(chunk_buffer_s=0.25)  # 4000 samples at 16 kHz
    core, _, _ = make_core(values=values)
    core.configure(CFG, now=0.0)
    events = await core.receive_audio(audio(8000), now=0.1)
...
# @spec ING-LIFE-002
async def test_no_input_after_final_reaches_the_engine() -> None:
    core, fake, _ = make_core()
    core.configure(CFG, now=0.0)
    await core.receive_audio(audio(CHUNK_SAMPLES), now=0.1)
    await core.finalize(now=0.2)
...
# @spec ING-LIFE-003
async def test_zero_audio_finalize_still_frees_the_slot() -> None:
    core, _, gate = make_core()
    core.configure(CFG, now=0.0)
    await core.finalize(now=0.1)
