...
Model, entrypoint and transport modules use normal imports from the selected
source pair. Engine execution is fake, and collection installs no package or
utility-module stubs that could leak into later CLI or server tests.
"""
...
def _load_chain() -> dict[str, Any]:
    """Import genuine modules without collection-time global replacements."""
    return {
        short: importlib.import_module(dotted)
        for dotted, short in (
            ("vllm_omni.metrics.streaming_transport", "streaming_transport"),
