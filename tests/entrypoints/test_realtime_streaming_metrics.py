...
def test_manager_projection_uses_the_same_installed_metrics_instance() -> None:
    ...
    assert state.openai_serving_realtime.runtime_config is state.persistent_state_service.runtime_config
...
def _serving_realtime(*, observer: Any, accepted_audio_budget_s: float | None) -> Any:
    ...
    serving.runtime_config = (
        None if accepted_audio_budget_s is None else SimpleNamespace(accepted_audio_budget_s=accepted_audio_budget_s)
    )
...
class _CapturingServing:
    ...
    def transcribe_realtime(self, audio_stream: Any, input_stream: Any, *, session_key: str | None = None) -> Any:
...
def test_park_token_resolves_for_the_recognized_streaming_model(
    ...
    monkeypatch.setattr(serving_mod, "_resolve_park_token_id", lambda hf_config: 777, raising=False)
