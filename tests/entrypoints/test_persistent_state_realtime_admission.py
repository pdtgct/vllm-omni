...
def _start_pre_admission_timeouts(connection: RealtimeConnection) -> None:
    ...
    if not callable(start):
        _fail("PORT-SESS-012 missing orthogonal pre-admission clock authority")
    start()


def _require_standard_update(websocket: _WebSocket) -> dict[str, Any]:
    updates = [event for event in websocket.sent if event.get("type") == "session.updated"]
    if len(updates) != 1:
        pytest.fail(
            f"PORT-SESS-010 expected exactly one standard session.updated event, observed {len(updates)}",
            pytrace=False,
        )
    return updates[0]
...
async def test_connection_entry_arms_the_total_unadmitted_lifetime(
    ...
    assert websocket.sent, "PORT-SESS-012 connection entry did not arm total unadmitted timeout"
...
async def test_audio_before_manager_ack_is_rejected_without_buffering() -> None:
    ...
    await connection.handle_event({"type": "input_audio_buffer.append", "audio": audio})
...
async def test_first_valid_update_reserves_before_marking_model_valid() -> None:
    ...
    await connection.handle_event({"type": "session.update", "model": "nemotron-asr"})
    ...
    assert call.get("service_interval_ms") == 560, "PORT-STATE-025 requires the admitted cadence at reserve"
...
async def test_standard_ack_follows_real_session_construction_and_observer_open(
    ...
    connection.serving.model_config = SimpleNamespace(hf_config=SimpleNamespace(endpoint_history_capacity_frames=12))
    ...
    monkeypatch.setattr(
        "vllm_omni.entrypoints.openai.realtime_connection.NemotronRealtimeSession.from_model_config",
        construct,
    )
...
async def test_audio_racing_admission_is_rejected_before_standard_ack() -> None:
    ...
    service.reserve = delayed_reserve  # type: ignore[method-assign]
    update = asyncio.create_task(connection.handle_event({"type": "session.update", "model": "nemotron-asr"}))
    await asyncio.sleep(0)
    audio = base64.b64encode(b"\x01\x00").decode()
    await connection.handle_event({"type": "input_audio_buffer.append", "audio": audio})
...
async def test_permitted_locale_update_acks_only_after_application() -> None:
    ...
    def record_locale(locale: str) -> None:
        assert websocket.sent == []
        applied.append(locale)

    # Exercise the real model-local authority. Its public operation is
    # synchronous select_prompt; update_locale belongs to the lease wrapper.
    session = object.__new__(NemotronRealtimeSession)
    session._prompts = {"auto": 0, "de-DE": 1}
    session._prompt_index = 0
    session._accepted_audio = SimpleNamespace(update_locale=record_locale)
    ...
    assert applied == ["de-DE"], "PORT-LID-001 / PORT-SESS-010 locale update was not applied"
    ...
    with pytest.raises(ValueError):
        await connection.handle_event({"type": "session.update", "model": "nemotron-asr", "locale": "xx-XX"})
    assert session.prompt_index == 1
...
def _service_error(name: str, *args: Any, **kwargs: Any) -> Exception:
    ...
    try:
        ...
        _fail(f"PORT-STATE-024 missing typed {name}")
    try:
        return error_cls(*args, **kwargs)
    except TypeError:
        _fail(f"PORT-STATE-024 {name} lacks the approved typed metadata")
...
async def test_retryable_shed_carries_backoff_and_keeps_socket_open() -> None:
    ...
    await connection.handle_event({"type": "session.update", "model": "nemotron-asr", "cadence": "320ms"})
...
async def test_unsupported_interval_is_nonretryable_and_keeps_socket_open() -> None:
    ...
    await connection.handle_event({"type": "session.update", "model": "nemotron-asr", "cadence": "80ms"})
...
async def test_total_unadmitted_timeout_releases_a_late_reserve_result() -> None:
    ...
    configure = asyncio.create_task(connection.handle_event({"type": "session.update", "model": "nemotron-asr"}))
...
async def test_app_state_inventories_service_before_install(
    ...
    async def prepare(**kwargs: Any) -> Any:
        snapshot = await kwargs["stage_client"].call_utility_async("persistent_state_snapshot")
...
async def test_failed_inventory_never_installs_service(
    ...
    async def prepare(**kwargs: Any) -> Any:
        return await kwargs["stage_client"].call_utility_async("persistent_state_snapshot")
