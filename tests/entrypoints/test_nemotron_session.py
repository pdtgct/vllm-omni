...
The selected core/Omni source pair is imported normally. Engine execution
remains fake; no global package or utility-module stubs are installed during
collection. The sampling-coercion recorder is patched only for its own test.
"""
...
#: Calls the per-test ``coerce_param_message_types`` recorder observes,
...
def _load_chain() -> dict[str, Any]:
    """Import genuine modules without replacing canonical package identities."""
    return {
        short: importlib.import_module(dotted)
        for dotted, short in (
            ...
            ("vllm_omni.entrypoints.nemotron_session", "nemotron_session"),
        )
    }


@pytest.fixture
def record_sampling_coercion(monkeypatch: pytest.MonkeyPatch) -> None:
    from vllm_omni.entrypoints import utils

    real_coerce = utils.coerce_param_message_types

    def record(params: list[Any], is_streaming: bool) -> list[Any]:
        _COERCE_CALLS.append((list(params), is_streaming))
        return real_coerce(params, is_streaming=is_streaming)

    monkeypatch.setattr(utils, "coerce_param_message_types", record)
...
class FakeAsyncOmni:
    ...
    def __init__(self, *, script: Any = None, stop_after: int | None = None) -> None:
        ...
        self.runtime_config: SimpleNamespace | None = SimpleNamespace(
...
def test_feed_projects_committed_segments_before_returning_park_hypothesis() -> None:
    async def scenario() -> None:
        ...
        observed: list[Any] = []
        hypotheses = await _wait(lease.feed(_audio(_CHUNK), on_segment=observed.append))
...
def test_feed_coerces_engine_default_sampling_for_streaming(record_sampling_coercion: None) -> None:
    async def scenario() -> None:
        engine = FakeAsyncOmni()
        lease, _ = _make_lease(engine)
        _COERCE_CALLS.clear()
...
def test_factory_lease_submits_engine_request_under_the_lease_session_key() -> None:
    ...
    async def scenario() -> None:
        engine = FakeAsyncOmni(script=lambda item, index: [_out([7, PARK_ID], text=" ok")])
        factory = NemotronSessionFactory(engine=engine)
        lease = await factory.open(cadence=_CADENCE, locale="en-US")
...
def test_render_factory_mirrors_core_parse_render_wrap(
    ...
    from vllm.engine import protocol
    from vllm.renderers.inputs import preprocess

    monkeypatch.setattr(protocol, "StreamingInput", FakeStreamingInput)
    monkeypatch.setattr(preprocess, "parse_model_prompt", parse_model_prompt)
