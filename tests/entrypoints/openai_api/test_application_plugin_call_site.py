...
from vllm.entrypoints.serve.instrumentator.metrics import attach_router as attach_metrics_router
...
async def test_selected_plugin_lifecycle_is_nested_around_serving_and_engine(monkeypatch) -> None:
    ...
    setattr(optional_entry_point, "config_optional", True)
...
def test_builtin_health_replacement_preserves_plugin_collision_detection() -> None:
    ...
    with pytest.raises(RuntimeError, match="operations route collision: /health"):
        api_server._install_and_validate_application_operations_routes(shadowed_app)
...
def test_vllm_metrics_route_pair_preserves_plugin_collision_detection() -> None:
    ...
    attach_metrics_router(shadowed_app)

    @shadowed_app.get("/metrics")
    async def foreign_metrics():
        return "foreign"
...
async def test_startup_deadline_is_not_reset_for_each_sequential_entry(
    ...
    setattr(entry_point, "config_optional", True)
    _install_worker_basics(monkeypatch, events, serve=must_not_serve)
    monkeypatch.setattr(
        api_server,
        "discover_application_plugins",
