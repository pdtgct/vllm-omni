...
async def test_later_nonfinal_native_commit_queues_forced_segment() -> None:
    ...
    await connection.handle_event({"type": "input_audio_buffer.commit", "final": False})
...
async def test_committed_segment_emits_additive_event_and_generation_continues() -> None:
    ...
    assert getattr(sent_events[-1], "terminal", None) is True
    segment_events = [event for event in sent_json if event.get("type") == "transcription.segment.done"]
    ...
    assert [
        (kind, getattr(payload, "type", None) or payload.get("type")) for kind, payload in connection._test_timeline
    ] == [
...
async def test_output_capacity_overflow_publishes_no_part_of_committed_result() -> None:
    engine = _RealtimeGenerationEngine([_generation_output("too-large", [7, 101])])
    connection, sent_events, sent_json, sent_errors = _realtime_generation_connection(engine)
    ...
    assert sent_events == []
    assert sent_errors == [("output_capacity_exceeded", "output_capacity_exceeded")]
    assert not any(event.get("type") == "transcription.segment.done" for event in sent_json)
...
async def test_duplicate_segment_generation_publishes_at_most_once() -> None:
    ...
    assert sum(event.get("type") == "transcription.segment.done" for event in sent_json) == 1
    assert sent_errors == []
