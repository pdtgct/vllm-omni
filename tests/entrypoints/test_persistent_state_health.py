...
def _request(engine: _Engine) -> SimpleNamespace:
    return SimpleNamespace(app=SimpleNamespace(state=SimpleNamespace(engine_client=engine)))
...
async def _health_response(engine: _Engine) -> Any:
    try:
        ...
        pytest.fail(
            f"PORT-STATE-030 native health leaked state-service failure instead of returning 503: {error}",
            pytrace=False,
        )
...
async def test_state_authority_loss_maps_to_503_instead_of_generic_500() -> None:
    ...
    engine = _Engine(
        PersistentStateServiceUnavailable("persistent-state authority is unhandshaked: secret-operation-id")
    )

    response = await _health_response(engine)

    assert response.status_code == 503
    assert response.body == (b'{"status":"unhealthy","reason":"persistent_state_unavailable"}'), (
