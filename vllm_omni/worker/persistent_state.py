...
def partition_persistent_state_config(
    ...
    # Core places all tensor records into one backing allocation. Splitting
    # mixed groups into separate allocations would change aliasing and budget.
    if ordinary_groups:
        raise ValueError("persistent-state allocation requires a persistent-only cache layout")
    ...
    for tensor in kv_cache_config.kv_cache_tensors:
        shared_by = set(tensor.layers)
        if shared_by & state_layer_names:
            if shared_by - state_layer_names:
                raise ValueError("persistent-state and ordinary layers cannot share an allocation")
            spec = state_group.kv_cache_spec
            if (
                tensor.layers != state_group.layer_names
                or tensor.offset != 0
                or tensor.block_stride != spec.page_size_bytes
                or tensor.size != kv_cache_config.num_blocks * spec.page_size_bytes
            ):
                raise ValueError("persistent-state allocation requires contiguous aggregate pages")
