...
class AdmissionControllerConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if not (1 <= self.dispatch_budget <= self.max_inflight_reserves <= self.waiter_capacity):
            raise ValueError("dispatch budget, reserve limit, and waiter capacity are ordered")
        ...
        if (
            ...
            raise ValueError("one to at most five supported intervals must be unique and positive")
...
class BoundedAdmissionController:
    ...
    def snapshot(self) -> AdmissionControllerSnapshot:
        ...
        return AdmissionControllerSnapshot(
            ...
            protected_attempt_id=(
                None if protected is None or protected.attempt is None else protected.attempt.attempt_id
            ),
    ...
    def _retry_after_ms(self, attempt_id: str) -> int:
        ...
        jitter = int.from_bytes(digest, "big") % (self._config.retry_jitter_ms + 1)
    ...
    def enqueue(self, attempt: AdmissionAttempt) -> AdmissionHandle:
        if attempt.service_interval_ms not in self._queue_heads:
            raise ValueError(f"unsupported service interval {attempt.service_interval_ms}")
        existing = self._connections.get(attempt.connection_id)
        if existing is not None and self._entry_or_none(existing) is not None:
            return existing
        if not self._free:
            raise AdmissionQueueFull(retry_after_ms=self._retry_after_ms(attempt.attempt_id))
        ...
        entry.deadline_closes_connection = attempt.unadmitted_deadline_ns <= attempt.admission_deadline_ns
    ...
    def expire_due(self) -> tuple[AdmissionDisposition, ...]:
        ...
        while self._deadline_size:
            ...
            outcome: AdmissionOutcome = "shed" if self._authority == "OPEN" else "unavailable"
            dispositions.append(
                self._disposition(
                    entry,
                    outcome,
    ...
    def drain(
        ...
        if protected is None:
            for interval in self._config.supported_intervals_ms:
                ...
                if now - entry.enqueued_at_ns >= self._config.aging_threshold_ns and (
                    protected is None or entry.sequence < protected.sequence
                ):
                    protected = entry
        ...
        while len(launched_items) < available:
            ...
            selected_index = -1
            for index, interval in enumerate(self._config.supported_intervals_ms):
                if remaining_dispatchable[index] <= 0:
                    continue
                entry = self._head(interval)
                if entry is not None and (selected is None or entry.sequence < selected.sequence):
                    selected = entry
    ...
    def engine_epoch_changed(
        ...
        entries = tuple(entry for entry in self._entries if entry.state != "free")
        dispositions = tuple(self._disposition(entry, "unavailable") for entry in entries)
        self._engine_epoch = engine_epoch
        self._authority = "UNHANDSHAKED"
        return dispositions
