...
class StagePool:
    ...
    async def abort_requests(self, request_ids: list[str]) -> None:
        ...
        try:
            ...
            # Output-processor state is independent of a live core binding.
            # Always clear it, including lifecycle-divergence cleanup where
            # the binding is already missing.
            if self._output_processor is not None:
                self._output_processor.abort_requests(
                    request_ids,
                    internal=True,
                )
