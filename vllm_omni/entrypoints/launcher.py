...
from vllm.entrypoints.launchers.launcher import (
    serve_http as _upstream_serve_http,
)
...
def request_application_shutdown(app_state: Any, engine: EngineClient, cause: BaseException | None = None) -> None:
    """Synchronously close selected application admission before engine stop."""
    engine_errored = engine.errored and not engine.is_running
    should_terminate = engine_errored and not envs.VLLM_KEEP_ALIVE_ON_ENGINE_DEATH
    if should_terminate:
        request = getattr(app_state, "request_application_shutdown", None)
        if request is not None:
            try:
                request(cause)
            except BaseException:
                logger.exception("application shutdown hook failed")
...
# vllm.entrypoints.launchers.launcher before the pin is accepted.
...
# SHA-256 of the vllm.entrypoints.launchers.launcher source every behavior above was last
...
UPSTREAM_LAUNCHER_SHA256 = "393c213ea68e12936e2ed018d4b6bc9368049bf0c26ab135790ac4fee15772ed"
...
async def _serve_http_with_lifecycle(
    ...
    *,
    ...
    ssl_cert_refresher = (
        None
        if (not enable_ssl_refresh or ssl_context is None or ssl_keyfile is None or ssl_certfile is None)
        else SSLCertRefresher(
            ssl_context=ssl_context,
            key_path=None if ssl_keyfile is None else os.fspath(ssl_keyfile),
            cert_path=None if ssl_certfile is None else os.fspath(ssl_certfile),
            ca_path=None if ssl_ca_certs is None else os.fspath(ssl_ca_certs),
        )
    )
...
def _build_server(
    ...
    setattr(config, "h11_max_header_count", h11_max_header_count)
    config.load()
