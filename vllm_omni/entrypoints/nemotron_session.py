...
if TYPE_CHECKING:
    ...
    FloatSamples: TypeAlias = npt.NDArray[np.float32]
...
class NemotronSessionLease:
    ...
    def __init__(
        ...
        *,
        ...
        if self._persistent_state_service is not None and self._state_lease is not None:
            timeout_s = float(self._persistent_state_service.pending_claim_timeout_s)
    ...
    async def feed(
        ...
        *,
        ...
        receipt: PieceReceipt | None = None
        try:
    ...
    async def release(self) -> None:
        ...
        async with self._state_cleanup_lock:
            ...
            self._observe_session_finished_once(
                self._terminal_reason or ("aborted" if self._aborted or not self._ended else "completed")
            )
            if (
    ...
    async def _rendered(self, render: Render) -> AsyncIterator[Any]:
        ...
        prompts = buffer_stream(
            ...
            on_audio_accepted=lambda: self._arm_session_lifecycle_timeout("idle"),
        )
        async for prompt in prompts:
            streaming_input = await render(prompt)
            if self._state_lease is not None:
                rendered_prompt = dict(streaming_input.prompt)
                information = dict(rendered_prompt.get("additional_information") or {})
            ...
            async for output in outputs:
                ...
                if completion is None and self._session.eou_token_id in ids:
                    ...
                    completion = SegmentCompletion(
                        ...
                        text="",
                        reason=("forced" if in_flight_kind == "forced_eou" else "model"),
                    )
                if completion is not None:
                    ...
                    if committed_completion is not None and self._active_segment_callback is not None:
                ...
                if self._park_id in ids:
                    ...
                    if in_flight_kind == "forced_eou":
                        ...
                        self._ledger.complete_next(self._session.transcript.complete_text)
    ...
    def __init__(
        ...
        *,
        ...
        if runtime_config is None and persistent_state_service is not None:
            runtime_config = getattr(persistent_state_service, "runtime_config", None)
        if persistent_state_service is not None and runtime_config is None:
            raise ValueError("persistent-state session factory requires the resolved streaming runtime envelope")
        ...
        *,
        ...
        if runtime is not None:
            session_limits = {
                ...
                "accepted_audio_capacity_samples": (runtime.accepted_audio_capacity_samples),
                "max_retained_transcript_bytes": (runtime.max_retained_transcript_bytes),
...
def create_nemotron_session_factory(engine_client: Any) -> SessionFactory:
    ...
    if not isinstance(architectures, (list, tuple)) or "Nemotron3_5AsrForRNNT" not in architectures:
        raise ValueError("Nemotron session factory requires the Nemotron3_5AsrForRNNT architecture")
