...
class _ObservedAudioBuffer(Protocol):
    def __call__(
        self,
        ...
        *,
        observer: Any,
        accepted_audio_budget_s: float | None,
...
class NemotronServingRealtime(OpenAIServingRealtime):
    ...
    ``OpenAIServingRealtime.transcribe_realtime`` at vLLM v0.29.0
    (``98dff2a81d747d1dba01a47f939f48c3526d4206``), except it passes the
    ...
    def _model_declares_widened_buffer_kwargs(self) -> bool:
        ...
        return "observer" in params and "accepted_audio_budget_s" in params and "session_key" in params
    ...
    async def transcribe_realtime(
        ...
        *,
        ...
        if self._model_declares_widened_buffer_kwargs:
            # The explicit signature gate above establishes this extension.
            buffered = cast(_ObservedAudioBuffer, self.model_cls.buffer_realtime_audio)(
                audio_stream,
                input_stream,
                model_config,
                observer=self._observer,
                accepted_audio_budget_s=(
                    self.runtime_config.accepted_audio_budget_s if self.runtime_config is not None else None
                ),
