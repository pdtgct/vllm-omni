...
class _SessionKeyTranscriber(Protocol):
    def __call__(
        self,
...
class _ResolvedEndpoint(TypedDict):
    mode: str
    stop_history_ms: int
    residue_frames: int
...
class RealtimeConnection(VllmRealtimeConnection):
    ...
    def __init__(self, *args, observer: Any = None, park_token_id: int | None = None, **kwargs):
        ...
        try:
            self._persistent_state_service = self.engine.get_persistent_state_service()
        ...
        if self._persistent_state_service is not None and runtime is None:
            raise ValueError("persistent-state serving requires its resolved runtime envelope")
        if service_runtime is not None and runtime is not service_runtime:
            raise ValueError("persistent-state serving and service runtime envelopes differ")
        self._runtime_config = runtime
        configured_timeout = runtime.session_configuration_timeout_s if runtime is not None else None
        self.session_configuration_timeout = float(30.0 if configured_timeout is None else configured_timeout)
        ...
        self._session_lifecycle = SessionLifecycleDeadline(
            idle_timeout_s=(runtime.session_idle_timeout_s if runtime is not None else None),
            finalization_timeout_s=(runtime.session_finalization_timeout_s if runtime is not None else None),
    ...
    def _start_pre_admission_timeouts(self) -> None:
        ...
        configuration_timeout_s = (
            self.session_configuration_timeout if runtime is None else float(runtime.session_configuration_timeout_s)
        )
        self.session_configuration_timeout = configuration_timeout_s
        total_timeout_s = configuration_timeout_s if runtime is None else float(runtime.unadmitted_connection_timeout_s)
        loop = asyncio.get_running_loop()
        self._unadmitted_deadline_ns = time.monotonic_ns() + int(total_timeout_s * 1_000_000_000)
    ...
    async def _transcribe_nemotron_realtime(
        ...
        # The native model yields an async generator; the generic core
        # SupportsRealtime declaration types this seam as a coroutine.
        stream = cast(
            AsyncGenerator[PromptType, None],
            self.serving.model_cls.buffer_realtime_audio(audio_stream, input_stream, session),
        )
        async for prompt in stream:
            parsed = parse_model_prompt(self.serving.model_config, prompt)
            (engine_input,) = await self.serving.renderer.render_cmpl_async([parsed])
    ...
    async def start_generation(self):
        ...
        if getattr(self, "_nemotron_session", None) is None:
            ...
            if "session_key" in inspect.signature(transcribe).parameters:
                streaming_input = cast(_SessionKeyTranscriber, transcribe)(
                    audio_stream,
                    input_stream,
            ...
            return
        native_request_id = self._state_session_key
        if native_request_id is None:
            raise RuntimeError("admitted Nemotron session has no correlation identity")
    ...
    async def handle_event(self, event: dict):
        ...
        if (
            ...
            return
        if event_type == "session.update" and getattr(self, "_persistent_state_service", None) is not None:
        ...
        if event_type == "input_audio_buffer.commit":
            if not self._is_model_validated:
                ...
                return
            generation_live = self.generation_task is not None and not self.generation_task.done()
    ...
    async def _configure_persistent_session_locked(self, event: dict) -> None:
        ...
        if self._is_model_validated:
            ...
            if locale is not None and self._nemotron_session is not None:
                self._nemotron_session.select_prompt(str(locale))
        ...
        try:
            ...
            lease = await service.reserve(
                ...
                admission_deadline_ns=(
                    time.monotonic_ns() + int(float(runtime.admission_wait_timeout_s) * 1_000_000_000)
                ),
            ...
            retry_after_ms = (
                int(runtime.admission_retry_floor_ms) if advertised_retry_ms is None else int(advertised_retry_ms)
            )
            await self.send_error(
                str(error),
                "capacity_exhausted",
            ...
            self._next_admission_retry_ns = time.monotonic_ns() + int(retry_after_ms * 1_000_000)
            deadline = self._unadmitted_deadline_ns
            if deadline is not None and self._next_admission_retry_ns >= deadline:
                self._is_connected = False
                close = getattr(self.websocket, "close", None)
                if close is not None:
                    await close(code=1013)
        ...
        resolved_endpoint: _ResolvedEndpoint = {
        ...
        try:
            ...
            if model_config is not None:
                runtime = self._runtime_config
                if runtime is None:
                    raise RuntimeError("admitted persistent-state session has no runtime envelope")
                ...
                endpoint_policy = EndpointPolicy.resolve(
                    ...
                    frame_stride_ms=80,
                    history_capacity_frames=int(getattr(hf, "endpoint_history_capacity_frames", 12)),
                )
                ...
                self._nemotron_session = NemotronRealtimeSession.from_model_config(
                    ...
                    accepted_audio_budget_s=float(runtime.accepted_audio_budget_s),
                    accepted_audio_capacity_samples=int(runtime.accepted_audio_capacity_samples),
                    max_retained_transcript_bytes=int(runtime.max_retained_transcript_bytes),
    ...
    def _completion_from_tokens(
        ...
        reason: Literal["forced", "model"] = "forced" if getattr(in_flight, "kind", None) == "forced_eou" else "model"
    ...
    async def _run_generation(
        ...
        *,
        ...
        transcript = BoundedTranscript(
            max_retained_bytes=int((1 << 20) if transcript_limit is None else transcript_limit),
        ...
        try:
            ...
            result_gen = self.engine.generate(
                ...
                request_id_already_unique=(getattr(self, "_state_lease", None) is not None),
            )
            ...
            async for output in result_gen:
                ...
                if stage_id == 0 and output.outputs:
                    ...
                    session = getattr(self, "_nemotron_session", None)
                    in_flight = session.accepted_audio.in_flight_unit if session is not None else None
                    if new_token_ids:
                    ...
                    if projection.delta:
                        await self.send(TranscriptionDelta(delta=projection.delta))
                    ...
                    if completion is not None:
                        segment = transcript.complete_segment(
                            ...
                            # The transcript authority validates nonterminal reasons.
                            reason=cast(Literal["model", "forced"], completion.reason),
                        )
            ...
            if observer is not None:
                ...
                remaining = observe_safely(observer.clear_all_outstanding, request_id, outcome=outcome)
    ...
    async def _expire_session_lifecycle(
        ...
        self._streaming_finish_reason = "aborted" if kind == "idle" else "error"
    ...
    async def _release_state_lease(
        ...
        if service is None or lease is None or operation_id is None or self._state_release_done:
            return False
    ...
    async def cleanup(self):
        ...
        await self._cancel_pre_admission_timeouts()
        await self._release_state_lease(getattr(self, "_terminal_code", None) or "connection_cleanup")
    ...
    async def send_error(
        ...
        *,
        ...
        await self.websocket.send_text(error.model_dump_json(exclude_none=True))
