...
class PersistentStateManager(SingleTypeKVCacheManager):
    ...
    def configure_capacity(
        ...
        *,
        ...
        if self._capacity_configured:
            if self.safety_reserve_slots != safety_reserve_slots or self.max_resident_sessions != max_resident_sessions:
                raise RuntimeError("persistent-state capacity configuration changed")
            return
        if self._bindings:
            raise RuntimeError("persistent-state capacity cannot change after admission")
    ...
    def remove_skipped_blocks(
        self,
        request_id: str,
        processed_computed_tokens: int | None = None,
        num_prompt_tokens: int | None = None,
        *,
        num_computed_tokens: int | None = None,
        total_computed_tokens: int | None = None,
    ) -> None:
        del request_id, processed_computed_tokens, num_prompt_tokens, num_computed_tokens, total_computed_tokens
