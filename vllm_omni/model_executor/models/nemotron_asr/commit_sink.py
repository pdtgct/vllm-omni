...
class BoundedCommitSink:
    ...
    def __init__(
        ...
        *,
        ...
        self._status_host = torch.zeros(max_rows, dtype=torch.int32, pin_memory=self._cuda)
    ...
    def reserve(self, plan: CommitPlan) -> CommitTicket:
        ...
        if self._reserved is not None or self._staged is not None:
            raise ValueError(
                "commit sink already holds an unconsumed reservation — collect or cancel the previous step first"
            )
        rows = len(plan.bindings)
        if rows > self._max_rows:
            raise ValueError(f"{rows} status rows exceed the sink bound {self._max_rows}")
        ...
        if plan.capture is not None:
            if plan.capture.rows > self._max_capture_rows:
                raise ValueError(f"{plan.capture.rows} capture rows exceed the sink bound {self._max_capture_rows}")
            if plan.capture.payload_bytes > self._max_capture_bytes:
                raise ValueError(
                    f"{plan.capture.payload_bytes} capture bytes exceed the sink bound {self._max_capture_bytes}"
                )
            if plan.capture.rows != len(plan.records):
                raise ValueError("capture plan row count does not match its frozen candidate records")
        elif plan.records:
            raise ValueError("capture-disabled commit plan carries candidate records")
        prior_row = -1
        for record in plan.records:
            ...
            if record.row <= prior_row or not 0 <= record.row < rows:
                raise ValueError("capture records must have unique increasing plan rows")
            binding = plan.bindings[record.row]
            if (
                record.request_id != binding.request_id
                or record.block_id != binding.block_id
                or record.admission_generation != binding.admission_generation
                or record.geometry != binding.geometry_id
            ):
                raise ValueError("capture record identity disagrees with its prepared binding")
        ...
        if plan.capture is not None:
            ...
            if plan.capture.payload_bytes != actual_payload_bytes:
                raise ValueError("capture plan payload bytes do not match its frozen records")
