...
class NemotronASRForRNNT(nn.Module):
    ...
    def __init__(self, *, vllm_config: Any = None, prefix: str = "") -> None:
        ...
        self._execution_memory_failed = False
    ...
    def _require_execution_inventory_ready(self) -> None:
        ...
        if binding is None:
            if getattr(self.config, "decode_dispatch_arm", None) == "dense-graphed":
                raise RuntimeError("decoder execution inventory is missing")
            return
    ...
    @torch.inference_mode()
    def prepare_execution_memory(self) -> None:
        """Retain execution inventory and profile its transient headroom before state allocation."""
        ...
        if self._execution_memory_failed:
            raise RuntimeError("execution memory preparation failed; a fresh worker is required")
        if self._execution_memory_prepared:
    ...
    def warmup_resident_state(self) -> None:
        """Warm actual resident scatter after execution memory was prepared."""

        from vllm_omni.model_executor.models.nemotron_asr.advance import (
            warmup_advance_model_rows_scatter,
        )

        if self._execution_memory_failed or not self._execution_memory_prepared:
            raise RuntimeError("execution memory inventory was not prepared successfully")
        self._require_execution_inventory_ready()
