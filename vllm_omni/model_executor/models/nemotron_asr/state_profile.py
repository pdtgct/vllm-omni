...
def build_nemotron_persistent_state_spec(config: Any) -> PersistentStateSpec:
    ...
    for entry in manifest["entries"]:
        ...
        if aligned != offset:
            raise ValueError(
                f"state manifest requires implicit padding before {entry['name']!r}; publish the padding explicitly"
            )
        init_name = entry["init"]
        initializer = _filled(blank_label) if init_name == "blank_label" else _zeros
...
def project_nemotron_state_pools(
    ...
    *,
    ...
    views = storage.views
    channels = tuple(views[f"encoder.layers.{index}.window.channel"] for index in range(n_layers))
    convolutions = tuple(views[f"encoder.layers.{index}.conv.time"] for index in range(n_layers))
    valid_lengths = tuple(views[f"encoder.layers.{index}.window.valid"] for index in range(n_layers))
