...
class ProjectionJoin:
    ...
    def complete(
        ...
        *,
        ...
        if not (record.mm_req_ids == record.binding_req_ids == batch_req_ids):
...
class _RequestMetadata:
    ...
    def snapshot(self, req_ids: tuple[str, ...]) -> dict[str, object]:
        try:
            ...
            raise RuntimeError("persistent-state request metadata is incomplete") from exc
...
class NemotronASRModelState(ModelState):  # type: ignore[misc]
    ...
    def reconcile_omni_lifecycle(
        ...
        *,
        ...
        self._initialized_binding_keys = {key for key in initialized if key[0] not in terminal_set}
    ...
    def prepare_dummy_inputs(self, num_reqs: int, num_tokens: int) -> dict[str, Any]:
        ...
        raise RuntimeError("Nemotron persistent state does not support CUDA graph capture dummy inputs")
    ...
    def prepare_attn(
        ...
        bindings_by_id = getattr(self._scheduler_output, "persistent_state_bindings", {})
        req_ids = tuple(input_batch.req_ids)
        try:
            source_bindings = tuple(bindings_by_id[req_id] for req_id in req_ids)
        except KeyError as exc:
            raise RuntimeError("scheduler omitted a persistent-state binding") from exc
        bindings = tuple(
            replace(
                binding,
                fresh=(binding.request_id, binding.generation) not in self._initialized_binding_keys,
            )
        ...
        self._projected_binding_keys = {(binding.request_id, binding.generation) for binding in bindings}
