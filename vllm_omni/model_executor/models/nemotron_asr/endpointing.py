...
class EndpointBook:
    ...
    def newest(self, count: int) -> tuple[FrameSymbol, ...]:
        ...
        return tuple(self.history[(start + offset) % len(self.history)] for offset in range(count))
    ...
    def append(self, symbol: FrameSymbol) -> EndpointBook:
        ...
        return replace(
            ...
            segment_has_output=(self.segment_has_output or symbol != FrameSymbol.BLANK),
        )
...
def frame_symbol(
    ...
    *,
    ...
    return FrameSymbol.WORD_START if is_word_start(int(labels[-1])) else FrameSymbol.NON_WORD_START
...
def observe_chunk(
    ...
    *,
    ...
    symbols = tuple(frame_symbol(labels, is_word_start=is_word_start) for labels in frames)
    candidate = book
    if not final_tail:
    ...
    trace = (
        _detect(candidate, policy) if policy.mode == "greedy_blank" and not final_tail else DetectorTrace(0, -1, 0, 0)
    )
    is_eou = (
