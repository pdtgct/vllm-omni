...
def load_publish_metadata(
    ...
    try:
        ...
        raise ValueError(f"cannot read valid publish metadata at {path}: {exc}") from exc
    ...
    for name, digest in (
        ...
        if not isinstance(digest, str) or not re.fullmatch(r"sha256:[0-9a-f]{64}", digest):
            raise ValueError(f"meta.json {name} must be a sha256:<64-hex> string")
...
def verify_dump_provenance(
    ...
    *,
    ...
    if source_checkpoint_digest != expected_source_digest:
        raise ValueError(
            f"source .nemo digest disagrees with dump metadata: {source_checkpoint_digest} != {expected_source_digest}"
        )
